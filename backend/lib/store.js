...
const VALID_DS_SQUAD_PREFS = ["hospital", "oil", "science-info", "roaming", "no_preference"];
function normalizeDesertStormSquadPreference(value) {
  return VALID_DS_SQUAD_PREFS.includes(value) ? value : "no_preference";
}
  ...
  return {
    ...
    responses: Array.isArray(value?.responses)
      ? value.responses.map((response) => ({
          ...
          preferredSquad: normalizeDesertStormSquadPreference(response.preferredSquad),
...
function createStore(config = {}) {
  ...
  function publicDesertStormVote(vote, players, viewerPlayerId = "", viewerIsLeader = false) {
    ...
    const base = {
      ...
      didVote: Boolean(response),
      selectedOptionId: response?.optionId || null,
      selectedPreferredSquad: response?.preferredSquad || "no_preference",
      votedAt: response?.createdAt || null
    };
    if (!viewerIsLeader) {
      return base;
    ...
    return {
      ...
      responses: (vote.responses || []).map((entry) => ({
        ...
        preferredSquad: entry.preferredSquad || "no_preference",
        createdAt: entry.createdAt
      }))
    };
  }
...
function getDraftedPlayerIdsForEvent(alliance, event) {
  ...
  function submitDesertStormVote(allianceId, eventId, player, optionId, preferredSquad) {
    const alliance = findAllianceById(allianceId);
    if (!alliance) throw new UserError("Alliance not found.");
    const event = findDesertStormEvent(alliance, eventId);
    if (!event) throw new UserError("Desert Storm event not found.");
    ...
    const normalizedPreferredSquad = normalizeDesertStormSquadPreference(preferredSquad);
    const existing = (event.vote.responses || []).find((entry) => entry.playerId === player.id);
    const previousOptionId = existing?.optionId || "";
    if (existing) {
      existing.optionId = optionId;
      existing.preferredSquad = normalizedPreferredSquad;
      ...
      event.vote.responses.push({
        ...
        playerName: player.name,
        optionId,
        preferredSquad: normalizedPreferredSquad,
        createdAt: new Date().toISOString()
      });
    }
  ...
  function autoGenerateDesertStormDraft(allianceId, eventId, player) {
    const alliance = findAllianceById(allianceId);
    if (!alliance) throw new UserError("Alliance not found.");
    const event = findDesertStormEvent(alliance, eventId);
    if (!event) throw new UserError("Desert Storm event not found.");
    if (event.status === "completed") throw new UserError("This Desert Storm event can no longer be edited.");

    const playerMap = new Map(alliance.players.map((p) => [p.id, p]));

    function buildPool(optionId) {
      return (event.vote.responses || [])
        .filter((r) => r.optionId === optionId)
        .map((r) => {
          const member = playerMap.get(r.playerId);
          return {
            playerId: r.playerId,
            playerName: r.playerName || member?.name || "",
            heroPower: member?.heroPower || 0,
            preferredSquad: r.preferredSquad || "no_preference"
          };
        })
    ...
    const playPool = buildPool("play");
    const subPool = buildPool("sub");
    const taskForces = clearTaskForceAssignments(clone(event.draftTaskForces));

    // Pick from pool: prefer players who requested this squad, fallback to highest power
    function pick(pool, squadPref) {
      if (squadPref && squadPref !== "no_preference") {
        const idx = pool.findIndex((p) => p.preferredSquad === squadPref);
        if (idx !== -1) return pool.splice(idx, 1)[0];
      }
      return pool.length ? pool.splice(0, 1)[0] : null;
    }

    function fillSquadSlots(tfKey, squadId, memberType, pool, squadPref) {
      const squad = (taskForces[tfKey]?.squads || []).find((s) => s.id === squadId);
      if (!squad) return;
      for (const slot of squad.slots.filter((s) => s.memberType === memberType && !s.playerName)) {
        const p = pick(pool, squadPref);
        if (!p) break;
        slot.playerName = p.playerName;
      }
    }

    // TASK FORCE A — best players first
    // 1. Roaming (top hero power)
    fillSquadSlots("taskForceA", "roaming", "Full-time", playPool, "roaming");
    // 2. Flex positions (next highest)
    fillSquadSlots("taskForceA", "hospital", "Flex", playPool, "no_preference");
    fillSquadSlots("taskForceA", "science-info", "Flex", playPool, "no_preference");
    // 3. Science — priority 1
    fillSquadSlots("taskForceA", "science-info", "Strategist", playPool, "science-info");
    fillSquadSlots("taskForceA", "science-info", "Full-time", playPool, "science-info");
    // 4. Oil — priority 2
    fillSquadSlots("taskForceA", "oil", "Strategist", playPool, "oil");
    fillSquadSlots("taskForceA", "oil", "Full-time", playPool, "oil");
    // 5. Hospital — priority 3
    fillSquadSlots("taskForceA", "hospital", "Strategist", playPool, "hospital");
    fillSquadSlots("taskForceA", "hospital", "Full-time", playPool, "hospital");

    // TASK FORCE B — remaining players; top 2 go to roaming to support newer players
    const tfbRoamingSquad = (taskForces.taskForceB?.squads || []).find((s) => s.id === "roaming");
    if (tfbRoamingSquad) {
      const slots = tfbRoamingSquad.slots.filter((s) => s.memberType === "Full-time");
      // First 2 slots: better players (support role)
      for (let i = 0; i < Math.min(2, slots.length); i++) {
        const p = pick(playPool, "roaming") || pick(playPool, null);
        if (p) slots[i].playerName = p.playerName;
      }
      // Remaining roaming slots: next available
      for (let i = 2; i < slots.length; i++) {
        if (!slots[i].playerName) {
          const p = pick(playPool, null);
          if (p) slots[i].playerName = p.playerName;
        }
      }
    }
    // TF-B science — priority 1
    fillSquadSlots("taskForceB", "science-info", "Strategist", playPool, "science-info");
    fillSquadSlots("taskForceB", "science-info", "Full-time", playPool, "science-info");
    fillSquadSlots("taskForceB", "science-info", "Flex", playPool, "science-info");
    // TF-B oil — priority 2
    fillSquadSlots("taskForceB", "oil", "Strategist", playPool, "oil");
    fillSquadSlots("taskForceB", "oil", "Full-time", playPool, "oil");
    // TF-B hospital — priority 3
    fillSquadSlots("taskForceB", "hospital", "Strategist", playPool, "hospital");
    fillSquadSlots("taskForceB", "hospital", "Full-time", playPool, "hospital");
    fillSquadSlots("taskForceB", "hospital", "Flex", playPool, "hospital");

    // Any leftover play voters fill remaining non-sub slots
    for (const tfKey of ["taskForceA", "taskForceB"]) {
      for (const squad of taskForces[tfKey]?.squads || []) {
        if (squad.id === "substitutes") continue;
        for (const slot of squad.slots || []) {
          if (!slot.playerName && playPool.length) {
            slot.playerName = playPool.splice(0, 1)[0].playerName;
          }
        }
      }
    }

    // Sub voters fill substitute slots (TF-A first, then TF-B)
    for (const tfKey of ["taskForceA", "taskForceB"]) {
      const subSquad = (taskForces[tfKey]?.squads || []).find((s) => s.id === "substitutes");
      if (!subSquad) continue;
      for (const slot of subSquad.slots || []) {
        if (!subPool.length) break;
        slot.playerName = subPool.splice(0, 1)[0].playerName;
      }
    }

    event.draftTaskForces = taskForces;
    event.hasUnpublishedChanges = true;
    event.voteChangedDraftedMemberIds = [];
    event.updatedAt = new Date().toISOString();
    commit();
