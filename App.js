...
export default function App() {
  ...
  const [newCalendarDate, setNewCalendarDate] = useState(new Date().toISOString().slice(0, 10));
...
function CalendarView({ entries, currentUserIsLeader, calendarView, onChangeCalendarView, newCalendarTitle, newCalendarDescription, newCalendarDate, newCalendarLeaderNotes, newCalendarLeaderOnly, onChangeNewCalendarTitle, onChangeNewCalendarDescription, onChangeNewCalendarDate, onChangeNewCalendarLeaderNotes, onToggleLeaderOnly, onCreateEntry, onDeleteEntry }) { const today = startOfLocalDay(); const weekEnd = new Date(today); weekEnd.setDate(weekEnd.getDate() + 7); const monthStart = new Date(today.getFullYear(), today.getMonth(), 1); const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 1); const visibleEntries = [...entries].filter((entry) => { const entryDate = startOfLocalDay(entry.startsAt); if (calendarView === "today") return isSameLocalDay(entryDate, today); if (calendarView === "week") return entryDate >= today && entryDate < weekEnd; return entryDate >= monthStart && entryDate < monthEnd; }).sort((a, b) => String(a.startsAt).localeCompare(String(b.startsAt))); return <View style={styles.card}><Text style={styles.cardTitle}>Alliance Calendar</Text><Text style={styles.hint}>Use Today, Week, or Month to see what matters right now.</Text><View style={styles.row}><Pressable style={[styles.secondaryButton, styles.half, calendarView === "today" && styles.modeButtonActive]} onPress={() => onChangeCalendarView("today")}><Text style={[styles.secondaryButtonText, calendarView === "today" && styles.modeButtonTextActive]}>Today</Text></Pressable><Pressable style={[styles.secondaryButton, styles.half, calendarView === "week" && styles.modeButtonActive]} onPress={() => onChangeCalendarView("week")}><Text style={[styles.secondaryButtonText, calendarView === "week" && styles.modeButtonTextActive]}>Week</Text></Pressable><Pressable style={[styles.secondaryButton, styles.half, calendarView === "month" && styles.modeButtonActive]} onPress={() => onChangeCalendarView("month")}><Text style={[styles.secondaryButtonText, calendarView === "month" && styles.modeButtonTextActive]}>Month</Text></Pressable></View>{currentUserIsLeader ? <View style={styles.section}><Text style={styles.sectionTitle}>Add Calendar Entry</Text><TextInput value={newCalendarTitle} onChangeText={onChangeNewCalendarTitle} style={styles.input} placeholder="Event title" /><TextInput value={newCalendarDate} onChangeText={onChangeNewCalendarDate} style={styles.input} placeholder="YYYY-MM-DD" autoCapitalize="none" /><TextInput value={newCalendarDescription} onChangeText={onChangeNewCalendarDescription} style={[styles.input, styles.textArea]} placeholder="What should members know or do?" multiline /><TextInput value={newCalendarLeaderNotes} onChangeText={onChangeNewCalendarLeaderNotes} style={[styles.input, styles.textArea]} placeholder="Leader-only notes" multiline /><View style={styles.row}><Pressable style={[styles.secondaryButton, styles.half, newCalendarLeaderOnly && styles.modeButtonActive]} onPress={onToggleLeaderOnly}><Text style={[styles.secondaryButtonText, newCalendarLeaderOnly && styles.modeButtonTextActive]}>{newCalendarLeaderOnly ? "Leader Only Entry" : "Visible To Everyone"}</Text></Pressable><Pressable style={[styles.button, styles.half]} onPress={onCreateEntry}><Text style={styles.buttonText}>Add To Calendar</Text></Pressable></View></View> : null}<View style={styles.section}><Text style={styles.sectionTitle}>{calendarView === "today" ? "Today's Priorities" : calendarView === "week" ? "This Week" : "This Month"}</Text>{visibleEntries.length ? visibleEntries.map((entry) => <View key={entry.id} style={styles.voteCard}><View style={styles.memberCardHeader}><View style={styles.memberHeaderText}><Text style={styles.sectionTitle}>{entry.title}</Text><Text style={styles.hint}>{String(entry.startsAt).slice(0, 10)}{entry.leaderOnly ? " • Leader Only" : ""}</Text></View>{currentUserIsLeader ? <Pressable style={styles.dangerButton} onPress={() => onDeleteEntry(entry.id)}><Text style={styles.dangerButtonText}>Delete</Text></Pressable> : null}</View>{entry.description ? <Text style={styles.line}>{entry.description}</Text> : null}{currentUserIsLeader && entry.leaderNotes ? <View style={styles.memberStatCard}><Text style={styles.memberStatLabel}>Leader Notes</Text><Text style={styles.line}>{entry.leaderNotes}</Text></View> : null}<Text style={styles.hint}>Added by {entry.createdByName || "Leader"}</Text></View>) : <Text style={styles.hint}>{calendarView === "today" ? "Nothing is scheduled for today." : calendarView === "week" ? "Nothing is scheduled for this week." : "Nothing is scheduled for this month."}</Text>}</View></View>; }
