...
export default function App() {
  ...
  const [actionNeededVisible, setActionNeededVisible] = useState(false);
  const [actionNeededSeenKey, setActionNeededSeenKey] = useState("");
  ...
  const unresolvedResponseItems = useMemo(() => {
    const items = votes
      .filter((vote) => vote.status === "open" && !vote.didVote)
      .map((vote) => ({
        id: vote.id,
        type: "vote",
        title: vote.title,
        subtitle: "Alliance vote response needed"
      }));
    zombieSiegeEvents
      .filter((event) => event.status !== "archived" && (event.myAvailabilityStatus || "no_response") === "no_response")
      .forEach((event) => {
        items.push({
          id: event.id,
          type: "zombieSiege",
          title: event.title,
          subtitle: "Zombie Siege availability response needed"
        });
      });
    return items;
  }, [votes, zombieSiegeEvents]);
  const unresolvedResponseKey = useMemo(() => unresolvedResponseItems.map((item) => `${item.type}:${item.id}`).join("|"), [unresolvedResponseItems]);
  ...
  useEffect(() => {
    ...
    if (!unresolvedResponseItems.length) {
      setActionNeededVisible(false);
      setActionNeededSeenKey("");
      return;
    }
    if (unresolvedResponseKey !== actionNeededSeenKey) {
      setActionNeededVisible(true);
      setActionNeededSeenKey(unresolvedResponseKey);
    }
  }, [session.token, alliance, unresolvedResponseItems, unresolvedResponseKey, actionNeededSeenKey]);
      ...
      <KeyboardAvoidingView style={styles.keyboardShell} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 12 : 0}>
        <View style={styles.screen}>
          ...
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"}>
            ...
            {activeTab === "feedback" ? <FeedbackView feedbackEntries={feedbackEntries} newFeedbackText={newFeedbackText} onChangeNewFeedbackText={setNewFeedbackText} onSubmitFeedback={() => run(async () => { await addFeedbackRequest(session.backendUrl, session.token, newFeedbackText); setNewFeedbackText(""); await refresh(); })} t={t} /> : null}
          </ScrollView>
        </View>
      </KeyboardAvoidingView>
      <Modal visible={actionNeededVisible} transparent animationType="fade" onRequestClose={() => setActionNeededVisible(false)}>
        <View style={styles.overlay}>
          <View style={styles.actionNeededModal}>
            <Text style={styles.cardTitle}>Action Needed</Text>
            <Text style={styles.hint}>You have responses waiting. Tap an item to go straight to it, or choose Later.</Text>
            <ScrollView contentContainerStyle={styles.section}>
              {unresolvedResponseItems.map((item) => (
                <Pressable
                  ...
                  onPress={() => {
                    ...
                    setSelectedZombieSiegeEventId(item.id);
            ...
            <Pressable style={styles.secondaryButton} onPress={() => setActionNeededVisible(false)}>
        ...
        <SafeAreaView style={styles.safeArea}>
          <KeyboardAvoidingView style={styles.keyboardShell} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 12 : 0}>
            <View style={styles.screen}>
              ...
              {playerModal && latestDesertStormVote && playerPickerMode === "voted" ? <Text style={styles.hint}>{playerModal.memberType === "Sub" ? `Showing members who voted "${DESERT_STORM_SUB_LABEL}" in ${DESERT_STORM_VOTE_TITLE}.` : `Showing members who voted "${DESERT_STORM_PLAY_LABEL}" in ${DESERT_STORM_VOTE_TITLE}.`}</Text> : null}
              ...
              <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"}>
                <Pressable style={styles.pick} onPress={() => run(async () => { await updateTaskForceSlot(session.backendUrl, session.token, { taskForceKey: playerModal.taskForceKey, squadId: playerModal.squadId, slotId: playerModal.slotId, playerName: "" }); setPlayerModal(null); await refresh(); })}><Text style={styles.pickText}>{t("clearSelection")}</Text></Pressable>
                {filteredOptions.map((player) => <Pressable key={player.id} style={styles.pick} onPress={() => run(async () => { await updateTaskForceSlot(session.backendUrl, session.token, { taskForceKey: playerModal.taskForceKey, squadId: playerModal.squadId, slotId: playerModal.slotId, playerName: player.name }); setPlayerModal(null); await refresh(); })}><Text style={styles.pickText}>{player.name} - {player.rank} - {player.overallPower.toFixed(2)}M</Text></Pressable>)}
                {!filteredOptions.length ? <Text style={styles.hint}>{playerPickerMode === "voted" && latestDesertStormVote ? t("noMembersMatchVoteFilter") : t("noPlayersMatchSearch")}</Text> : null}
              </ScrollView>
            </View>
          </KeyboardAvoidingView>
        </SafeAreaView>
