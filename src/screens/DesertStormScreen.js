...
const SQUAD_PREF_OPTIONS = [
  { id: "no_preference", label: "No Preference" },
  { id: "roaming", label: "Roaming" },
  { id: "science-info", label: "Science" },
  { id: "oil", label: "Oil" },
...
export function DesertStormScreen({
  ...
  useEffect(() => {
    setPreferredSquad(selectedEvent?.vote?.selectedPreferredSquad || "no_preference");
  ...
  return <View style={styles.section}>
    ...
    {selectedEvent && currentUserIsLeader ? <>
      ...
      {(section === "taskForceA" || section === "taskForceB") ? <AppCard styles={styles}>
        ...
        <SecondaryButton label="Auto Generate Teams" onPress={() => Alert.alert("Auto Generate Teams", "This will clear the current draft and auto-assign all players who voted Play or Sub based on hero power and squad preferences. Continue?", [{ text: "Cancel", style: "cancel" }, { text: "Generate", onPress: () => onAutoGenerateDraft(selectedEvent.id) }])} styles={styles} />
      </AppCard> : null}
...
function buildMemberStatusList(selectedEvent, players) {
  ...
  return (players || []).map((player) => {
    ...
    const squadPref = response.preferredSquad && response.preferredSquad !== "no_preference" ? response.preferredSquad : null;
    const squadLabel = squadPref === "roaming" ? "Roaming" : squadPref === "science-info" ? "Science" : squadPref === "oil" ? "Oil" : squadPref === "hospital" ? "Hospital" : null;
    return {
      id: player.id,
      name: player.name,
      detail: `${player.rank} - ${optionLabel}${squadLabel ? ` (${squadLabel})` : ""}`,
      badge: optionLabel,
