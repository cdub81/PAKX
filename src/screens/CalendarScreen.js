...
export function CalendarScreen(props) {
  const {
    ...
    calendarTranslationBackfillInFlight,
    calendarView,
    ...
    onToggleLeaderOnly,
    newCalendarSendPushReminder,
    onToggleSendPushReminder,
    onCreateEntry,
    onCancelEdit,
    onEditEntry,
    onDeleteEntry,
  ...
  const [showOriginalByEntryId, setShowOriginalByEntryId] = useState({});
  ...
  function toggleOriginalText(entryId) {
    setShowOriginalByEntryId((current) => ({
      ...current,
      [entryId]: !current[entryId]
    }));
  }

  function getEntryTranslationView(entry) {
    const sourceLanguage = String(entry.sourceLanguage || "en").toLowerCase();
    const targetLanguage = String(language || "en").toLowerCase();
    const requestedTranslation = sourceLanguage !== targetLanguage ? entry?.translations?.[targetLanguage] : null;
    const hasTranslatedTitle = Boolean(String(requestedTranslation?.title || "").trim());
    const hasTranslatedDescription = Boolean(String(requestedTranslation?.description || "").trim());
    const hasTranslatedText = hasTranslatedTitle || hasTranslatedDescription;
    const showOriginal = Boolean(showOriginalByEntryId[entry.id]);
    const translationStatus = String(entry.translationStatus || "idle").toLowerCase();

    return {
      sourceLanguage,
      translationStatus,
      hasTranslatedText,
      shouldShowToggle: sourceLanguage !== targetLanguage && hasTranslatedText,
      showOriginal,
      primaryTitle: hasTranslatedTitle ? requestedTranslation.title : entry.title,
      primaryDescription: hasTranslatedDescription ? requestedTranslation.description : entry.description,
      originalTitle: entry.title,
      originalDescription: entry.description,
      showPendingState: sourceLanguage !== targetLanguage && !hasTranslatedText && translationStatus === "pending",
      showUnavailableState: sourceLanguage !== targetLanguage && !hasTranslatedText && (translationStatus === "failed" || translationStatus === "disabled")
    };
  }
  ...
  return <View style={styles.section}>
    ...
    <AppCard style={styles.calendarDetailCard} styles={styles}>
      ...
      {selectedEntries.length ? <View style={styles.calendarAgendaList}>{selectedEntries.map((entry) => {
        ...
        const badgeLabel = entry.linkedType === "desertStorm" ? calendarT("linkedDesertStorm") : entry.linkedType === "zombieSiege" ? calendarT("linkedZombieSiege") : entry.leaderOnly ? calendarT("leaderOnly") : entry.allDay !== false ? calendarT("allDay") : calendarT("timeSpecificEntry");
        ...
        return <Pressable key={entry.occurrenceId || entry.id} style={styles.calendarEntryCard} disabled={!entry.linkedType} onPress={() => entry.linkedType && onOpenLinkedEntry(entry)}>
          ...
          {translationView.showUnavailableState ? <Text style={styles.hint}>{calendarT("translationUnavailable")}</Text> : null}
          {getRepeatLabel(entry) ? <Text style={styles.hint}>{getRepeatLabel(entry)}</Text> : null}
          {entry.allDay === false ? <View style={styles.calendarTimeStack}><ListRow title={calendarT("serverTime")} detail={entry.serverDisplayDateTime || entry.serverDisplayTime} styles={styles} /><ListRow title={calendarT("memberLocalTime")} detail={entry.localDisplayDateTime || entry.localDisplayTime} styles={styles} /></View> : null}
          {translationView.shouldShowToggle ? <SecondaryButton label={translationView.showOriginal ? calendarT("hideOriginalText") : calendarT("showOriginalText")} onPress={() => toggleOriginalText(entry.id)} styles={styles} /> : null}
          {translationView.shouldShowToggle && translationView.showOriginal ? <AppCard style={styles.calendarLeaderNotesCard} styles={styles}><Text style={styles.statusEyebrow}>{calendarT("originalText")}</Text><Text style={styles.line}>{translationView.originalTitle}</Text>{translationView.originalDescription ? <Text style={styles.line}>{translationView.originalDescription}</Text> : null}</AppCard> : null}
          {currentUserIsLeader && entry.leaderNotes ? <AppCard style={styles.calendarLeaderNotesCard} styles={styles}><Text style={styles.statusEyebrow}>{calendarT("leaderNotes")}</Text><Text style={styles.line}>{entry.leaderNotes}</Text></AppCard> : null}
          <View style={styles.cardHeaderRow}>
            <Text style={styles.hint}>{calendarT("addedBy", { name: entry.createdByName || t("calendar.leaderFallback") })}</Text>
            {currentUserIsLeader ? <View style={styles.memberCardActions}><SecondaryButton label={calendarT("edit")} onPress={() => onEditEntry(entry)} styles={styles} /><Pressable style={styles.dangerButton} onPress={() => Alert.alert("Delete Event", `Are you sure you want to delete "${entry.title}"? This cannot be undone.`, [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: () => onDeleteEntry(entry.sourceEntryId || entry.id) }])}><Text style={styles.dangerButtonText}>{calendarT("delete")}</Text></Pressable></View> : null}
          </View>
        </Pressable>;
      })}</View> : <AppCard style={styles.calendarEmptyCard} styles={styles}><Text style={styles.statusTitle}>{calendarView === "today" ? calendarT("nothingToday") : calendarT("tapAnotherDay")}</Text><Text style={styles.hint}>{calendarT("noEventsScheduled")}</Text></AppCard>}
    </AppCard>

    {currentUserIsLeader ? <PrimaryButton label={calendarT("addEntry")} onPress={onOpenCalendarForm} styles={styles} /> : null}
    {currentUserIsLeader ? <SecondaryButton label={calendarTranslationBackfillInFlight ? calendarT("backfillTranslationsWorking") : calendarT("backfillTranslations")} onPress={onBackfillTranslations} styles={styles} /> : null}
    <CalendarTimePickerModal visible={calendarTimePickerTarget === "start" || calendarTimePickerTarget === "end"} title={calendarTimePickerTarget === "end" ? calendarT("pickEndTime") : calendarT("pickStartTime")} value={calendarTimePickerTarget === "end" ? newCalendarEndTime : newCalendarStartTime} minValue={calendarTimePickerTarget === "end" && newCalendarEndDate === newCalendarDate ? newCalendarStartTime : ""} onChange={(nextValue) => { if (calendarTimePickerTarget === "end") { onChangeNewCalendarEndTime(nextValue); } else { onChangeNewCalendarStartTime(nextValue); } }} onClose={() => onChangeCalendarTimePickerTarget("")} language={language} t={t} />
    <CalendarDatePickerModal visible={calendarDatePickerTarget === "startDate" || calendarDatePickerTarget === "endDate" || calendarDatePickerTarget === "repeatEndDate"} title={calendarDatePickerTarget === "repeatEndDate" ? calendarT("repeatEndDate") : calendarDatePickerTarget === "endDate" ? calendarT("endDate") : calendarT("pickDate")} value={calendarDatePickerTarget === "repeatEndDate" ? (newCalendarRepeatEndDate || newCalendarDate) : calendarDatePickerTarget === "endDate" ? newCalendarEndDate : newCalendarDate} onChange={(nextValue) => { if (calendarDatePickerTarget === "repeatEndDate") { onChangeNewCalendarRepeatEndDate(nextValue); } else if (calendarDatePickerTarget === "endDate") { onChangeNewCalendarEndDate(nextValue); } else { onChangeNewCalendarDate(nextValue); } }} onClose={() => onChangeCalendarDatePickerTarget("")} language={language} t={t} />
  </View>;
}
