...
export function submitDesertStormVote(baseUrl, token, eventId, optionId, preferredSquad) {
  return request(baseUrl, `/api/desert-storm/events/${encodeURIComponent(eventId)}/vote/respond`, {
    ...
    body: JSON.stringify({ optionId, preferredSquad })
  });
}

export function autoGenerateDesertStormDraft(baseUrl, token, eventId) {
  return request(baseUrl, `/api/desert-storm/events/${encodeURIComponent(eventId)}/auto-generate`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`
...
export function getAlliancePushReachability(baseUrl, token) {
  return request(baseUrl, "/api/leader-controls/push-reachability", {
    headers: {
      Authorization: `Bearer ${token}`
    }
  });
